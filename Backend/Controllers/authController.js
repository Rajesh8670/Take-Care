...
const getOtp = async (req, res) => {
  try {
    ...
    try {
      ...
      if (emailError.code === "BREVO_CONFIG_INVALID" && allowDevOtpFallback) {
        console.warn("⚠️ Using development OTP fallback mode");
        return res.status(201).json({
          message: "OTP generated for development fallback mode.",
          email: normalizedEmail,
          devOtp: otp,
          deliveryMode: "local-fallback",
        });
      }

      delete otpStore[normalizedEmail];
      
      let configMessage = "OTP email delivery failed. Please try again later.";
      if (emailError.code === "BREVO_CONFIG_INVALID") {
        configMessage = "OTP email is not configured correctly on the server.";
      } else if (emailError.code === "BREVO_SENDER_INVALID") {
        configMessage = "OTP sender email is not verified in Brevo. Contact support.";
      } else if (emailError.code === "BREVO_INIT_FAILED") {
        configMessage = "Email service initialization failed. Contact support.";
      }
    ...
    console.error("\n❌ Unexpected Error in OTP Generation");
...
const checkOtp = (req, res) => {
  try {
    ...
    if (parseInt(userOtp, 10) === storedOtp.otp) {
...
const createAccount = async (req, res) => {
  try {
    ...
    if (!hasVerifiedOtp(normalizedEmail)) {
      return res
        .status(403)
        .json({ message: "Please verify OTP before creating your account" });
    }
...
const login = async (req, res) => {
  try {
    ...
    if (!isMatch) {
      return res
        .status(401)
        .json({ message: "Incorrect password", isLogin: false });
    }
...
const resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (!hasVerifiedOtp(normalizedEmail)) {
      return res
        .status(403)
        .json({ message: "Please verify OTP before resetting your password" });
    }
    ...
    return res
      .status(500)
      .json({ message: "Error During setpassord in DB" });
  }
};
